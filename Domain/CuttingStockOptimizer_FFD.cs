...
{
    public class CuttingStockOptimizer_FFD
    {
        /// <summary>
        /// 주어진 재고와 주문에 대해 최적의 절단 계획을 수립하는 메인 함수.
        /// First Fit Decreasing (FFD) 알고리즘 사용으로 근사해 구함.
        /// </summary>
        ...
        public static (List<(int StockLength, List<int> Cuts)> Result, List<int> Leftover, int Welds) OptimizeCutting(
            List<RebarStock> stock, List<Order> orders, float alpha, float beta, int gamma)
        {
            ...
            // 주문을 길이 기준 내림차순으로 정렬
            var sortedOrders = orders.OrderByDescending(o => o.Length).SelectMany(o => Enumerable.Repeat(o.Length, o.Quantity)).ToList();
        ...
        protected static void Example()
        {
            ...
            // 최적화 실행
            var (result, leftover, welds) = OptimizeCutting(stock, orders, alpha, beta, gamma);
